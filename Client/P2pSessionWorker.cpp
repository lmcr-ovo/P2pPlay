...
P2pSessionWorker::P2pSessionWorker(QObject* parent)
    ...
    connect(&transport_, &P2pUdpTransport::frameReady,
            this, &P2pSessionWorker::onFrameReady);
...
void P2pSessionWorker::setClientInfo(const QString &roomId, const QString &clientId) {
    roomId_ = roomId;
    clientId_ = clientId;
}

void P2pSessionWorker::setServerUdpEndpoint(const QHostAddress &address, quint16 port) {
    serverUdpAddress_ = address;
    serverUdpPort_ =  port;
}

void P2pSessionWorker::setPunchPortRange(quint16 range) {
    punchPortRange_ = range;
}

void P2pSessionWorker::onProbePermitted(const SignalingMessage &message) {
    if (!message.success) {
...
void P2pSessionWorker::onPeerEndpoint(const SignalingMessage &message) {
    if (!message.success) {
        emit errorOccurred(message.reason);
        return;
    }
...
void P2pSessionWorker::sendProbe() {
...
void P2pSessionWorker::sendPunch() {
    if (p2pReady_) {
        punchTimer_.stop();
        return;
    }
...
QList<quint16> P2pSessionWorker::punchCandidatePorts() const {
    QList<quint16> ports;
...
void P2pSessionWorker::handlePunch(const UdpFrame &frame) {
    UdpPunchPayload payload;
...
void P2pSessionWorker::handlePunchAck(const UdpFrame &frame) {
...
// 锁定对端udp port
void P2pSessionWorker::markP2pReady(const QHostAddress &address, quint16 port) {
    if (p2pReady_) {
        return;
    }
    p2pReady_ = true;
