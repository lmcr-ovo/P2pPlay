...
| $\alpha$ | IDR 相对码率理论帧的倍数（理论 α） | 无量纲 | 实测 $24 \sim 28$ |
| $k_{pace}$ | 发送速率/码率 的倍数 | 无量纲 | $30$（$> \alpha$） |
| $\tau$ | 自适应评估周期 | ms | $1000$ |
...
其中 $N_{kf}$ 与 $p$ 均由 guest 通过 ReceiverReportSample **同时上报**；$N_{kf}\ge3$ 更严重（腰斩），$p\ge p_{high}$ 温和降，$p$ 落在死区内不处理。
...
graph LR
    subgraph Guest
        RM[ReceiverMonitor<br/>丢帧率统计] -->|ReceiverReportSample 每τ| RC
    end
    subgraph Host
...
- 每 $\tau$ 结算一次，发 `ReceiverReportSample`，清零重计
- **注意**：seq 回绕（quint32 溢出）用环形差值 `& 0xFFFFFFFF` 处理
- 统计位置选在到达层（到达即算收到），避免解码器"等关键帧主动丢帧"干扰

### 3. 反馈报文 ReceiverReportSample

`UdpFrameType` 新增 `ReceiverReportSample = 9`，payload 结构：
...
| 文件 | 职责 |
|---|---|
| `Common/ReceiverReportSample.h/.cpp` | 反馈报文结构 + 编解码 |
...
| 文件 | 改动 |
|---|---|
| `Udp/UdpPacket.h` | `UdpFrameType` 加 `ReceiverReportSample` |
...
1. **反馈通道**：`UdpFrameType::ReceiverReportSample` + `ReceiverReportSample` 编解码
...
| guest → host | 丢帧率反馈 | `ReceiverReportSample`（新增） | `ReceiverMonitor` 每 τ | `RateController.onReceiverReport` |
